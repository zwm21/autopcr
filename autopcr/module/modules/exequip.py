                    ...
                    for star in ex_equip_group_by_star:
                        ...
                        for ex in ex_equip_group_by_star[star]:
                            ...
                            if is_rainbow:
                                ...
                                # 直接传入 sub_status，由 get_unit_attribute 内部处理
        ...
        if output_mode == '输出EX装备情况':
            for unit_id in slot_strategy:
                ...
                for (_, slot, ex_equipment_id, star, serial_id) in slot_strategy[unit_id]:
                    ...
                    attr = db.ex_equipment_data[ex_equipment_id].get_unit_attribute(star)
                    panel_parts = []
                    for key in UnitAttribute.__annotations__:
                        if UnitAttribute.is_present.get(key, False):
                            val = getattr(attr, key)
                            if val:
                                ch_name = UnitAttribute.index2ch.get(
                                    next((k for k, v in UnitAttribute.index2name.items() if v == key), None), key)
                                panel_parts.append(f"{ch_name}x{float(val)/100:.2f}%")
                    panel_str = '/'.join(panel_parts) if panel_parts else ''
                    is_rainbow = db.get_ex_equip_rarity(ex_equipment_id) == 5
                    if serial_id is not None and is_rainbow:
                        sub_str = db.get_ex_equip_sub_status_str(ex_equipment_id, client.data.ex_equips[serial_id].sub_status or [])
                        entry = f"{name}★{star}[{panel_str}]({sub_str})" if panel_str else f"{name}★{star}({sub_str})"
                    else:
                        entry = f"{name}★{star}[{panel_str}]" if panel_str else f"{name}★{star}"
                    msg.append(entry)
            ...
            for unit_id in client.data.unit:
                ...
                if unit_id in slot_strategy:
                    for (_, slot, ex_equipment_id, star, serial_id) in slot_strategy[unit_id]:
                        # 直接传入 sub_status（非彩装时为 None 或空列表，不影响结果）
                        sub_status = client.data.ex_equips[serial_id].sub_status if serial_id is not None else None
                        attr = db.ex_equipment_data[ex_equipment_id].get_unit_attribute(star, sub_status)
